...
class ChatlogsParserConfigWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.date_layout.setSpacing(spacing)
        ...
        self.fetch_history_checkbox = QCheckBox("Fetch history")
    ...
    def _on_mode_changed(self, index: int):
        ...
        if mode == "Single Date":
            ...
            self._add_date_input("From:", "from_date")
            self._add_date_input("To:", "to_date")
    ...
    def _on_mention_date_mode_changed(self, index: int):
        ...
        if sub_mode == "Single Date":
            ...
            self._add_date_input("From:", "mention_range_from")
            self._add_date_input("To:", "mention_range_to")
    ...
    def _add_date_input(self, label_text: str, obj_name: str):
    ...
    def update_progress(self, start_date: str, current_date: str, percent: int):
        ...
        self.progress_label.setText(f"📅 Start: {start_date} | Current: {current_date} | Progress: {percent}%")
    ...
    def _build_parse_config(self) -> Optional[ParseConfig]:
        ...
        if mode == "Single Date":
            ...
            from_input = self.findChild(QLineEdit, "from_date")
            to_input = self.findChild(QLineEdit, "to_date")
            if not from_input or not to_input or not from_input.text().strip() or not to_input.text().strip():
                print("Please enter both dates")
                return None
            from_date = from_input.text().strip()
            to_date = to_input.text().strip()
            ...
            if sub_mode == "Single Date":
                ...
                from_input = self.findChild(QLineEdit, "mention_range_from")
                to_input = self.findChild(QLineEdit, "mention_range_to")
                if not from_input or not to_input or not from_input.text().strip() or not to_input.text().strip():
                    print("Please enter both dates")
                    return None
                from_date = from_input.text().strip()
